...
class LightParamEdit(DataEditor):
    def setup_widgets(self):
        self.color1 = self.add_color_input("Light 1", "color1", with_alpha=True)
        for i in self.color1:
            i.setToolTip(ttl.lightparam["Light"])
        self.unkvec = self.add_multiple_decimal_input("Source Vector", "unkvec", ["x", "y", "z"],
                                                      -inf, +inf)
        for i in self.unkvec:
            i.setToolTip(ttl.lightparam["Position"])
        self.color2 = self.add_color_input("Ambient Light Color", "color2", with_alpha=True)
